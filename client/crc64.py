...
Table and algorithm follow akintos' InsomniacArchive (Crc64.cs, spiderman_pc
branch), by way of the ALERT tools (dat1lib/crc64.py, GPL). Vendored so the
client has no dependency outside this folder. Verified against the game: the
crc64 of a save-schema name is the value the save stores beside it.
...
SEED = 0xC96C5795D7870F42


def normalize(text):
    """Lower-case, backslashes to slashes, runs of slashes collapsed -- the
    same normalisation the engine applies before hashing a path."""
